...
import { createRandomChinese, randomHex } from '@/utils/utils'
async function initDb() {
  await db.targets.clear()
  let count = 30
  while (count >= 0) {
    const item = { remainder: Math.floor(Math.random() * 100 + 1), title: createRandomChinese(Math.random() * 6 + 1), time: new Date().getTime(), total: Math.floor(Math.random() * 100 + 1), color: randomHex() }
    const id = await db.targets.add(item)
    count--
  }
}
initDb()
...
function initPicture() {
  ...
  lists.forEach(async (v) => {
    const item = { src: v.src, introduce: v.introduce }
...
function initBanner() {
  ...
  bannerLists.forEach(async (v) => {
    const banner = { src: v.src, introduce: v.introduce }
